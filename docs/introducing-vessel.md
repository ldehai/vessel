...
First real numbers (Ubuntu 24.04, x86_64, CH v45, n=10): full-boot cold start averages 529ms — CH process spawn, kernel boot, agent handshake, exec. Forking a running sandbox (snapshot + restore every call) averages 230ms. The number that matters for agent workloads — restoring a session from a cached template snapshot — averages **143ms with ±1ms variance**, session-ready including an exec round-trip. The remaining 43ms to the sub-100ms target has a clear path: a prewarmed VMM pool and on-demand memory paging. Comparative numbers against Kata, E2B and microsandbox will accompany the first release. Also pending: the containerd shim, erofs image layering, and vsock socket remapping for many-clones-from-one-snapshot.
