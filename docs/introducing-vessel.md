...
Real numbers (Ubuntu 24.04, x86_64, Cloud Hypervisor v52, 256MiB template, n=10): full-boot cold start averages 521ms — CH process spawn, kernel boot, agent handshake, exec. The numbers that matter for agent workloads: restoring a session from a cached template snapshot averages **70ms** (best 58ms), session-ready including an exec round-trip, and thanks to userfaultfd on-demand paging that figure is independent of template memory size. Forking a *running* sandbox — pause, snapshot, restore a clone — is 86ms. And because each clone restores from its own hardlinked snapshot overlay with a private vsock path, clones parallelize: **ten concurrent sessions from one template are all ready in 173ms, an effective 17ms per session**. All reproducible with two scripts in the repo. Comparative numbers against Kata, E2B and microsandbox will accompany the first release. Also pending: the containerd shim and erofs image layering.
