...
First real numbers (Ubuntu 24.04, x86_64, CH v45, n=10): full-boot cold start averages 524ms — CH process spawn, kernel boot, agent handshake, exec. The number that matters for agent workloads — restoring a session from a cached template snapshot, served by a prewarmed VMM pool — averages **79ms with ±0.5ms variance** on a 128MiB template, session-ready including an exec round-trip. Sub-100ms, measured, reproducible with two scripts in the repo. Restore latency currently scales linearly with template memory (it is a straight NVMe read of the snapshot's memory file), so the v0.2 headline is on-demand paging to decouple the two. Comparative numbers against Kata, E2B and microsandbox will accompany the first release. Also pending: the containerd shim, erofs image layering, and vsock socket remapping for many-clones-from-one-snapshot.
